...
if [ "$build_tool" = "dpkg-buildpackage -b" ]; then
  apt-get -y --allow-unauthenticated install libclass-singleton-perl libpoe-perl libdatetime-timezone-perl libdatetime-locale-perl libfilter-perl liblist-moreutils-perl libparams-validate-perl libset-infinite-perl libdatetime-set-perl libdatetime-perl libpoe-perl
fi
